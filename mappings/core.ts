...
import { updatePairDayData, updateTokenDayData, updateShibcakeDayData, updatePairHourData } from "./dayUpdates";
...
export function handleSync(event: Sync): void {
  ...
  let shibcake = ShibcakeFactory.load(FACTORY_ADDRESS);

  // reset factory liquidity by subtracting only tracked liquidity
  shibcake.totalLiquidityBNB = shibcake.totalLiquidityBNB.minus(pair.trackedReserveBNB as BigDecimal);
  ...
  shibcake.totalLiquidityBNB = shibcake.totalLiquidityBNB.plus(trackedLiquidityBNB);
  shibcake.totalLiquidityUSD = shibcake.totalLiquidityBNB.times(bundle.bnbPrice);
...
export function handleMint(event: Mint): void {
  ...
  shibcake.totalTransactions = shibcake.totalTransactions.plus(ONE_BI);
...
export function handleSwap(event: Swap): void {
  ...
  let shibcake = ShibcakeFactory.load(FACTORY_ADDRESS);
  shibcake.totalVolumeUSD = shibcake.totalVolumeUSD.plus(trackedAmountUSD);
  shibcake.totalVolumeBNB = shibcake.totalVolumeBNB.plus(trackedAmountBNB);
  shibcake.untrackedVolumeUSD = shibcake.untrackedVolumeUSD.plus(derivedAmountUSD);
  shibcake.totalTransactions = shibcake.totalTransactions.plus(ONE_BI);
  ...
  let shibcakeDayData = updateShibcakeDayData(event);
  let token0DayData = updateTokenDayData(token0 as Token, event);
  let token1DayData = updateTokenDayData(token1 as Token, event);

  // swap specific updating
  shibcakeDayData.dailyVolumeUSD = shibcakeDayData.dailyVolumeUSD.plus(trackedAmountUSD);
  shibcakeDayData.dailyVolumeBNB = shibcakeDayData.dailyVolumeBNB.plus(trackedAmountBNB);
  shibcakeDayData.dailyVolumeUntracked = shibcakeDayData.dailyVolumeUntracked.plus(derivedAmountUSD);
  shibcakeDayData.save();
